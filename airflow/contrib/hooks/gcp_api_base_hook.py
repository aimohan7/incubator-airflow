    ...
    def _authorize(self):
        ...
        if 'Proxy_conn_id' in self.extras:
               ...
               if 'Proxy_type' in proxy_info:
                  ...
                  proxy_type = None
               if 'Proxy_host' in proxy_info:
                  proxy_host = proxy_info["Proxy_host"]
               else:
                  proxy_host = 'None'
               if 'Proxy_port' in proxy_info["Proxy_port"]:
                  proxy_port = int(proxy_info["Proxy_port"])
               else:
                  proxy_port = None
               if 'Proxy_rdns' in proxy_info:
                  ...
                  proxy_rdns = True
               if 'Proxy_user' in proxy_info: 
                  proxy_user = self.extras["Proxy_user"]
               else:
                  proxy_user = None
               if 'Proxy_pass' in proxy_info:
